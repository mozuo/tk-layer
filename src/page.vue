...
<div class="notify" @mousemove="move" @mouseup="moveEnd">
    ...
    <div :id="options.id + '_alert'" class="notify-main notify-alert"  :style="{left:options.offset[0] + 'px',top:options.offset[1] +'px', margin:options.offset[2]}">
        ...
        <div class="notify-btns">
            <tk-button btn="primary" @click.native="btnyes" size="small">确定</tk-button>
            <tk-button btn="default" @click.native="btncancel" size="small" v-if="typeof(options.cancel) == 'function' || options.cancel=='cancel'">取消</tk-button>
        </div>
    </div>
</div>
</template>
